...
function logout(){
  ...
  <section class="sidebar">
    <ul class="sidebar-menu">
        ...
        <?php else: ?>
              <li><a href="<?php echo url('index/admin/cinemaindex'); ?>"><i class="fa fa-bar-chart"></i> <span>管理概况</span></a></li>
              <li><a href="<?php echo url('Index/admin/ordercheck'); ?>"><i class="fa fa-bar-chart"></i>订单管理</a></li>
...
if(!isNaN('<?php echo $eq; ?>')){
  ...
  <div class="box box-solid">
    ...
    <div class="box-body">
      <form method="GET" action="<?php echo url('index/admin/ordercheck'); ?>" id='form'>
      <div class="row">
        <div class="col-xs-4 col-md-3">
          <div class="input-group">
            <input name="word" type='text' class='form-control' value="<?php echo $_GET['word']; ?>" placeholder='订单号关键词搜索..'>
            <span class="input-group-addon" onclick="$('#form').submit();"><i class="fa fa-search"></i></span>
            <?php if($_GET['word'] != ''): ?>
            <a title='清除条件' class="input-group-addon" href="<?php echo url('index/admin/ordercheck'); ?>"><i class="fa fa-remove"></i></a>
            <?php endif; ?>
          </div>
        </div>
      </div>
    ...
    <div class="box-body">
      <table class="table table-bordered">
        <tbody>
          ...
          <tr>
            <td><?php echo $vo['id']; ?></td><td><?php echo $vo['title']; ?></td><td><?php echo $vo['nickname']; ?></td>
            <td><?php echo $vo['phone']; ?></td><td><span class="label label-success">￥<?php echo $vo['money']; ?></span></td>
            <td><?php echo $vo['orderid']; ?></td><td><?php echo $vo['name']; ?></td><td><?php echo $vo['time']; ?></td>
            <td><?php if($vo['status'] == '2'): ?>已支付<?php elseif($vo['status'] == '3'): ?>已出票<?php endif; ?></td>
            ...
            <td>
              <div class="btn-group">
                ...
                <ul class="dropdown-menu" role="menu">
                  <li><a href="javascript:dealorder(<?php echo $vo['id']; ?>)">出票</a></li>
                  <li class="divider"></li>
                  <li><a href="javascript:del(<?php echo $vo['id']; ?>);">删除</a></li>
                </ul>
              </div>
            </td>
          </tr>
...
function dealorder(id){
  ...
  $.post(location.href,{'id':id},function(ret){
    alert(ret.msg);
    location.reload();
  },'JSON');
...
function del(id){
  if(confirm('Are you sure to delete this item from the list?') == true){
    $.post("<?php echo url('index/admin/del'); ?>",{'id':id},function(ret){
      if(ret.code ==1){
        location.reload();
      }
    },'JSON');
